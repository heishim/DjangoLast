...
{
	int i;
    /*ENTETE_TABLE_2019 entete_table;*/
    char *ptoffft;/*FG 11e:TEM*/

    /*FG 11e:TEM
    fseek(ptfidx_lp,offsettable,SEEK_SET);
    fread(entete_table.nomt,sizeof(ENTETE_TABLE_2019),1,ptfidx_lp);*/

	/*FG 11e:TEM
    for(i=0;i<entete_table.nstb;i++)
		accstb_ccam_gc_2019(ptfidx_lp,&sstab_info_2019[i],offsettable);
    */
    ptoffft=h_ptfidx_gc_2019+offsettable+sizeof(struct ET_2019);
    for(i=0;i<*(h_ptfidx_gc_2019+offsettable+27);i++)
    {
        sstab_info_2019[i].taille=*(ptoffft+20)*256+*(ptoffft+21);
        sstab_info_2019[i].offset=carlong_2019(ptoffft+26)+offsettable;
        sstab_info_2019[i].lgtot=carlong_2019(ptoffft+22);
        ptoffft+=sizeof(ENTETE_SSTAB_2019);
    }
...
int est_gc_2019(char code[])
{
	...
	if((buf = (unsigned char *)malloc(sstab_info_2019[3].lgtot))==0)
	{
		return(-1);
	}

	fpos = sstab_info_2019[3].offset;
	/*FG 11e:TEM
    fseek(ptfidx_gc_2019,fpos,0);
	fread(buf,sstab_info_2019[3].lgtot,1,ptfidx_gc_2019);*/
    memcpy(buf,h_ptfidx_gc_2019+fpos,sstab_info_2019[3].lgtot);

	i=0;
    while((i< sstab_info_2019[3].lgtot) && (memcmp(buf+i,code_binaire,7)!=0))
		i+=sstab_info_2019[3].taille;

	free(buf);
	if(i< sstab_info_2019[3].lgtot)
		return 0;
	else
		return 1;
...
int liste_compl_2019(char code_in[],char phase,char activite,char **res)
{
	...
	if(hc<0 || hc>=_nbpref_ccam_2019)
	{
		return(-1);
	}

	hc *= sstab_info_2019[0].taille;
	/*FG 11e:TEM
    fseek(ptfidx_gc_2019,sstab_info_2019[0].offset+hc,0);
	fread(sufx,sstab_info_2019[0].taille,1,ptfidx_gc_2019);*/
    memcpy(sufx,h_ptfidx_gc_2019+sstab_info_2019[0].offset+hc,sstab_info_2019[0].taille);
	...
	if((buf = (unsigned char *)malloc(nb_sufx*sstab_info_2019[1].taille))==0)
	{
		return(-1);
	}

	fpos=sstab_info_2019[1].offset+offset_sufx*sstab_info_2019[1].taille;
	/*FG 11e:TEM
    fseek(ptfidx_gc_2019,fpos,0);
	fread(buf,nb_sufx*sstab_info_2019[1].taille,1,ptfidx_gc_2019);*/
    memcpy(buf,h_ptfidx_gc_2019+fpos,nb_sufx*sstab_info_2019[1].taille);
	...
	i=0;
	while((i< nb_sufx * sstab_info_2019[1].taille) && (memcmp(buf+i,hc_term,5)!=0))
		i+=sstab_info_2019[1].taille;

	if(i<nb_sufx*sstab_info_2019[1].taille)
	{
		memcpy(info_st2,buf+i+5,sstab_info_2019[1].taille - 5);
	}
    else
    {
		free(buf);
	...
	if ((*res = (char *)malloc(TAILLE_ACTE_C_2019*nb_gest+3))==NULL)
	{
		free(buf);
		return(-1);
	}
	...
	*(*res)=nb_gest;
	for(i=0;i<nb_gest;i++)
	{
		fpos=sstab_info_2019[2].offset+i*sstab_info_2019[2].taille+offset_ind_gc*sstab_info_2019[2].taille;
		/*FG 11e:TEM
        fseek(ptfidx_gc_2019,fpos,0);
		fread(ind_lst_gc,2,1,ptfidx_gc_2019);*/
        memcpy(ind_lst_gc,h_ptfidx_gc_2019+fpos,2);

		offset_lst_gc = ind_lst_gc[0]*256+ind_lst_gc[1];

		fpos=sstab_info_2019[3].offset+offset_lst_gc*sstab_info_2019[3].taille;
		/*FG 11e:TEM
        fseek(ptfidx_gc_2019,fpos,0);
		fread(buf2,7,1,ptfidx_gc_2019);*/
        memcpy(buf2,h_ptfidx_gc_2019+fpos,7);

		*(*res+1+i*TAILLE_ACTE_C_2019) = (char)((buf2[0]*256+buf2[1])/676+'A');
		*(*res+2+i*TAILLE_ACTE_C_2019) = (char)(((buf2[0]*256+buf2[1])%676)/26+'A');
		*(*res+3+i*TAILLE_ACTE_C_2019) = (char)(((buf2[0]*256+buf2[1])%676)%26+'A');

		*(*res+4+i*TAILLE_ACTE_C_2019) = (char)(buf2[2]+'A');

		sprintf(*res+5+i*TAILLE_ACTE_C_2019,"%5.5i",buf2[3]*256*256*256+buf2[4]*256*256+buf2[5]*256+buf2[6]);
	}
